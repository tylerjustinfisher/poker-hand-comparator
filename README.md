...
# Contact
Tyler Fisher (tjfisher[at]gmail[dot]com)
Written Feb 2017
...
This will randomly generate a five-card board, plus two sample hands of two cards each.
It'll tell you which one won, and what each hand is.

The other scripts in this repo enable you to enter specific hands through the terminal.
The scripts in the heads-up folder are for a two-player game.
The scripts in the multiplayer folder are for games with more than three players.

Each of these scripts run on simple text inputs and outputs.
The outputs should all be straightforward enough to decipher.

If a script has a weird name and looks strange, it's because I was playing code golf with it.
That is, I was trying to make it as short as possible, for a contest with a friend.
It's not really meant to be legible, just functional in as few lines (or characters) as possible.
Indeed, that friend's code can be found in the file `ekshesh-code.py`.
...
# Symbols and card-related information:
